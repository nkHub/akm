...
当前实现已经优先接入第三方 `markdown-chunker`，把它作为默认 Markdown 切片器；它会优先保留标题、表格、代码块和列表等结构边界。考虑到本项目仍需要兼容“依赖未安装”或“第三方切片异常”的运行环境，插件内部仍保留本地 fallback，回退到原先的“标题优先 + 段落累计 + 字符级兜底”逻辑。

当前默认切片规则：

1. 优先使用 `markdown-chunker` 做结构感知切片
2. `chunk_size` 映射为 `soft_max_len`，作为目标 chunk 大小
3. `chunk_overlap` 用于放宽 `hard_max_len` 和估算 `min_chunk_len`，尽量减少把同一结构块过早切断
4. 若第三方切片不可用，再回退到“先按标题拆 section，再按段落累计，最后字符级兜底”的旧逻辑

这么做的原因是：

- Markdown 最有价值的信息就是标题层级与块级结构
- 纯字符切片容易破坏代码块、表格和列表边界
- 标题与结构信息对后续结果展示和可解释性更友好
- 保留 fallback 可以避免把索引可用性强绑定到外部依赖安装状态
