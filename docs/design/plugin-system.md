...
- **首页插件卡片插槽（通用）**：`dashboard.html` 在 mk 卡片区之后预留 `plugin_cards_html` 占位，由 `_build_plugin_dashboard_cards_section`（server.py）遍历「已启用且实现 `dashboard_card() -> dict`」的插件，逐张渲染为 `akm-plugin-card`；`_render_plugin_dashboard_card` 按协议字段拼卡：`id`（去重）/ `icon`（`_plugin_svg_icon` 支持名）/ `title` / `metrics`（[{label,value}] 主指标网格）/ `recent_title` + `recent`（[{ts,text,detail}] 最近事件行）/ `empty_text`（无记录占位）/ `actions`（[{label,href}] 跳转）。无任何可用卡片时整区返回空串。插件返回 HTML 不做转义，与既有知识库记忆卡先例一致（插件本就以本地代码运行）。设计目的：插件增删改首页卡片**只发插件更新，无需升级宿主内核**；旧内核不识别该区、旧插件无 `dashboard_card` 均自动跳过，双向兼容。首个采用者是 data_filter_guard v0.1.4（脱敏/还原/拦截统计卡，见其 README）；markdown_kb 记忆卡仍走下述专用实现。
