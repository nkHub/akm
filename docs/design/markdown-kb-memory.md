...
- 信号抽取严格化（v0.1.4）：工作区信号只在会话开头 3 条消息内识别——opencode 取完整
  `<env>...</env>` 块内成行的 `Working directory:`，Claude 取 system / 早期消息文本里的
  `Primary working directory`，对话正文（含被整段重发的历史消息）里的同类字样不再命中；
  抽取值先做路径清洗（只收绝对路径，拒绝夹带 `<` `>` 反引号、全角括号、省略号、引号等
  残片的取值），清洗为空则整体不命中；命中后若目录不存在或已删除，跳过自动建项目与
  后台刷新（不产生垃圾项目、不烧 token）；后台自动刷新在 KB 材料为空时直接跳过，
  避免生成“全缺失”占位文档；
...
| 对话正文出现 `Working directory:` 字样（非 env 块 / 超出前 3 条窗口） | v0.1.4 起不命中：只认开头 3 条内完整 `<env>` 块与 system 级信号 |
| 信号目录已删除 / 不存在 | v0.1.4 起不自动建项目；后台自动刷新对无 KB 材料仓库直接跳过 |
