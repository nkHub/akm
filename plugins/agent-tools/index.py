"""AKM 离线工具箱 Web 界面插件（agent-tools）。

将同级 tools 项目的构建产物打进本插件 dist/ 目录，启用后即可在
`<route_prefix>`（默认 /tools）访问，无需配置绝对路径。该工具集为
纯前端实现（JSON/YAML 格式化、时间戳、哈希、二维码、IP 查询等），
除 IP 查询外不依赖任何后端 API。
"""
...
class _SpaStaticFiles(StaticFiles):
    ...
    async def __call__(self, scope, receive, send):
        ...
        if not self._plugin.enabled or not self._plugin.runtime_ready:
            await PlainTextResponse("agent-tools 插件当前不可用", status_code=503)(scope, receive, send)
            return
...
class Plugin(PluginBase):
    """把内置的 tools 构建产物挂载为离线工具箱 Web 界面。"""
    ...
    async def on_load(self):
        ...
        if not build_dir.is_dir():
            self.logger.warning("[agent-tools] 未找到内置构建产物目录: %s", build_dir)
            return False

        index_file = build_dir / "index.html"
        if not index_file.is_file():
            self.logger.warning("[agent-tools] 未找到入口文件: %s", index_file)
            return False

        app = self.app
        if app is None:
            self.logger.warning("[agent-tools] FastAPI app 未注入，跳过挂载")
            return False

        # 清除同名旧挂载，避免进程内重启后旧实例残留路由遮蔽新实例。
        mount_names = {f"agent_tools_{self.name}"}
    ...
    def _route_prefix(self) -> str | None:
        ...
        if route_prefix == "/" or any(
            ...
            self.logger.warning("[agent-tools] 不允许使用受保护路径: %s", route_prefix)
            return None
        return route_prefix
