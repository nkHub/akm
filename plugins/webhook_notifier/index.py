...
class Plugin(PluginBase):
    ...
    2. 可选原生 App 系统通知：经 PluginBase.notify（菜单栏 rumps）弹出 macOS 通知。
    """

    router = router

    async def on_load(self):
        """初始化进程内去重表、后台任务与最近事件环形缓冲。"""
        global _plugin_instance
        _plugin_instance = self
        self._last_sent: dict[str, float] = {}
        self._tasks: set[asyncio.Task] = set()
        self._last_audit_queue_dropped = 0
        # 最近事件：仅作 status/recent 回看，不再推 SSE / 浏览器
        self._event_seq = 0
    ...
    def _settings(self) -> dict:
        ...
        cfg = self.config or {}
        # 兼容旧配置键 browser_notifications：若新键未写过，则沿用旧值语义
        if "app_notifications" in cfg:
            app_notifications = cfg.get("app_notifications") is True
        elif "browser_notifications" in cfg:
            app_notifications = cfg.get("browser_notifications") is True
        else:
        ...
        return {
            ...
            # 原生 App 通知默认开启：需通过菜单栏启动（python -m akm.menubar / 打包 App）
    ...
    def _resolve_notify(self):
        """解析宿主 notify 回调：优先实例注入，其次 app.state.host_notify。"""
        fn = getattr(self, "notify", None)
        if callable(fn):
            return fn
        state = getattr(getattr(self.app, "state", None), "host_notify", None)
        if callable(state):
            return state
        return None
    ...
    def _build_message(self, event: str, subtype: str, response: dict) -> tuple[str, str, dict]:
        """生成标题、纯文本正文与结构化 details（Webhook 与 App 通知共用）。"""
        status = int(response.get("status_code", 0) or 0)
        latency = int(response.get("latency_ms", 0) or 0)
        details = {
            "event": event,
    ...
    def _record_and_notify_app(self, event: str, subtype: str, response: dict) -> dict:
        """写入最近事件缓冲，并在可用时弹出原生 App 系统通知。"""
        title, text, details = self._build_message(event, subtype, response)
        self._event_seq += 1
        item = {
            "id": self._event_seq,
        ...
        notify_fn = self._resolve_notify()
        if notify_fn is None:
            self.logger.debug(
                "[webhook_notifier] 宿主 notify 未注入，跳过 App 通知（请用菜单栏启动）"
            )
            return item

        # subtitle 用事件类型；message 截断避免 macOS 通知过长
        body_lines = [
            f"{details['model'] or '-'} / {details['key_alias'] or '-'}",
            f"{details['api_path'] or '-'} · {details['status_code']} · {details['latency_ms']}ms",
            (details["error"] or details["security_reason"] or subtype)[:180],
        ]
        message = "\n".join(body_lines)
        try:
            notify_fn(title, f"{event}/{subtype}", message)
        except Exception as exc:
            self.logger.warning("[webhook_notifier] App 通知发送失败: %s", exc)
        return item
    ...
    def status(self) -> dict:
        ...
        return {
            ...
            "app_notifications": cfg["app_notifications"],
            "host_notify_available": self._resolve_notify() is not None,
    ...
    async def on_response(self, ctx):
        """按事件类型和冷却窗口决定是否发送 Webhook / App 原生通知。"""
        ...
        app_ok = cfg["app_notifications"]
        # 两条通道都未配置时直接跳过，避免无意义的事件分类
        if not webhook_ok and not app_ok:
            return None
        ...
        for event, subtype, event_response in events:
            ...
            # App 原生通道：不走网络，不占用 max_pending；无宿主时仅记缓冲
            if app_ok:
                try:
                    self._record_and_notify_app(event, subtype, event_response)
                except Exception as exc:
                    self.logger.warning("[webhook_notifier] App 通知发布失败: %s", exc)
