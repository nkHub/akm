...
class ResponsesAdapter(BaseAdapter):
    ...
    def convert_request(self, body: dict) -> dict:
        ...
        # 占位文本写成明确的续推指令 "[continue]"：模型只会理解成「继续」，
        # 不会被当成需要回应的新用户请求而反复复述（旧占位 "[tool output processed]"
        # 会被模型当成待处理事项反复提起）。
    ...
    def _input_to_messages(
        ...
        for item in input_value:
            ...
            if item_type == "function_call":
                ...
                # 占位用续推指令 "[continue]"（见 convert_request 中 tool 结尾追加逻辑注释）
                text = item.get("text", "") or "[continue]"
