...
async def _handle_ai_request(request: Request, api_path: str):
    ...
    try:
            ...
            async def stream_generator():
                ...
                try:
                    ...
                    await _submit_audit_log(request.app, {
                        ...
                        "key_alias": key_alias,
                        "model": model,
                        "request_body": request_body_for_log if (save_request_body and adapter and request_body_for_log) else (_safe_request_body_for_log(body) if (save_request_body and adapter) else ""),
                        "response_body": body_str if (save_response_body and adapter) else "",
                        "status_code": status,
                        "latency_ms": latency,
                        "error": stream_error or (f"{security_action}:{security_reason}" if security_action else ""),
                        "request_headers": request_headers_for_log if save_request_body else "",
                        "prompt_tokens": tokens.get("prompt_tokens", 0),
                        "completion_tokens": tokens.get("completion_tokens", 0),
                        "total_tokens": tokens.get("total_tokens", 0),
                        "cached_tokens": tokens.get("cached_tokens", 0),
                        "cache_creation_tokens": tokens.get("cache_creation_tokens", 0),
                        "client_request_headers": client_request_headers_for_log if save_request_body else "",
        ...
        await _submit_audit_log(request.app, {
            ...
            "key_alias": result["key_alias"],
            "model": result["model"],
            "request_body": request_body_for_log if (save_request_body and result.get("adapter") and request_body_for_log) else (_safe_request_body_for_log(body) if (save_request_body and result.get("adapter")) else ""),
            "response_body": result["body"] if (save_response_body and result.get("adapter")) else "",
            "status_code": result["status_code"],
