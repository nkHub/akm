...
class TestResponsesAdapterSSE:
    ...
    @pytest.mark.asyncio
    async def test_convert_sse_mcp_namespace_extraction(self):
        """验证 MCP 命名空间工具名被正确还原为短名 + namespace 字段

        发送方向 _convert_tools() 将 namespace 展开为全名（如 mcp__lazyweb__lazyweb_health），
        接收方向 convert_sse_stream() 应反向提取 namespace 并输出原生 Codex 格式。
        """
        adapter = ResponsesAdapter()
        # 模拟 _convert_tools() 已执行，namespace_map 已填充
        adapter._namespace_map = {
            "mcp__lazyweb__lazyweb_health": "mcp__lazyweb__",
            "mcp__memory__search_nodes": "mcp__memory__",
        }
        chat_sse = [
            b'data: {"id":"chatcmpl-mcp","model":"deepseek-v4","choices":[{"delta":{"role":"assistant"},"index":0}]}\n\n',
            b'data: {"id":"chatcmpl-mcp","choices":[{"delta":{"tool_calls":[{"index":0,"id":"call_mcp_1","type":"function","function":{"name":"mcp__lazyweb__lazyweb_health","arguments":"{}"}}]},"index":0}]}\n\n',
            b'data: {"id":"chatcmpl-mcp","choices":[{"delta":{"tool_calls":[{"index":1,"id":"call_mcp_2","type":"function","function":{"name":"mcp__memory__search_nodes","arguments":"{\\"query\\""}}]},"index":0}]}\n\n',
            b'data: {"id":"chatcmpl-mcp","choices":[{"delta":{"tool_calls":[{"index":1,"function":{"arguments":"\\":\\"test\\"}"}}]},"index":0}]}\n\n',
            b'data: {"id":"chatcmpl-mcp","choices":[{"finish_reason":"tool_calls"}],"usage":{"prompt_tokens":10,"completion_tokens":5,"total_tokens":15}}\n\n',
        ]
        ...
        # 检查 function_call output_item.added 事件的 namespace 字段
        fc_added = [e for e in events if e[0] == "response.output_item.added" and e[1]["item"]["type"] == "function_call"]
        assert len(fc_added) == 2

        # 第一个 tool: lazyweb_health（短名）+ namespace
        fc1 = fc_added[0][1]["item"]
        assert fc1["name"] == "lazyweb_health"
        assert fc1["namespace"] == "mcp__lazyweb__"
        assert fc1["type"] == "function_call"
        assert "fc_" in fc1["id"]  # 独立 item ID
        assert fc1["call_id"] == "call_mcp_1"  # 原始 call_id

        # 第二个 tool: search_nodes（短名）+ namespace
        fc2 = fc_added[1][1]["item"]
        assert fc2["name"] == "search_nodes"
        assert fc2["namespace"] == "mcp__memory__"
        assert fc2["call_id"] == "call_mcp_2"

        # 检查 sequence_number 存在
        assert "sequence_number" in fc_added[0][1]

        # 检查 response.completed 的 output 也包含 namespace
        completed = [e for e in events if e[0] == "response.completed"][0]
        completed_items = [o for o in completed[1]["response"]["output"] if o["type"] == "function_call"]
        assert len(completed_items) == 2
        c1 = completed_items[0]
        assert c1["name"] == "lazyweb_health"
        assert c1["namespace"] == "mcp__lazyweb__"
        c2 = completed_items[1]
        assert c2["name"] == "search_nodes"
        assert c2["namespace"] == "mcp__memory__"

        # 检查 function_call_arguments.done 也使用短名
        args_done = [e for e in events if e[0] == "response.function_call_arguments.done"]
        assert len(args_done) == 2
        assert args_done[0][1]["name"] == "lazyweb_health"
        assert args_done[1][1]["name"] == "search_nodes"
        # args_done 也应有 item_id
        assert "item_id" in args_done[0][1]

    @pytest.mark.asyncio
    async def test_convert_sse_non_mcp_no_namespace(self):
        """验证非 MCP 工具（无 namespace 映射）不输出 namespace 字段"""
        adapter = ResponsesAdapter()
        chat_sse = [
            b'data: {"id":"chatcmpl","model":"deepseek","choices":[{"delta":{"role":"assistant"},"index":0}]}\n\n',
            b'data: {"id":"chatcmpl","choices":[{"delta":{"tool_calls":[{"index":0,"id":"call_1","type":"function","function":{"name":"bash","arguments":"{\\"cmd\\":\\"ls\\"}"}}]},"index":0}]}\n\n',
            b'data: {"id":"chatcmpl","choices":[{"finish_reason":"tool_calls"}],"usage":{"prompt_tokens":5,"completion_tokens":2,"total_tokens":7}}\n\n',
        ]
        ...
        item = fc_added[0][1]["item"]
        assert item["name"] == "bash"
        assert "namespace" not in item  # 非 MCP 工具不应有 namespace 字段
