...
`data_filter_guard` 现已迁入内置插件目录 `akm/plugins/data_filter_guard/`，并保持“内置但默认关闭”；需要时可在插件页手动开启。它作为“数据安全插件”，目前提供三类能力：

- 按字段名递归脱敏，如 `api_key`、`authorization`、`password`
- 按关键词/正则规则替换文本内容，如手机号、内部代号、敏感短语
- 对非流式响应执行基础高风险内容扫描，支持整条拦截或局部替换明显恶意脚本/命令片段
- 对响应规则支持动作分级：`warn` / `mask` / `block`；流式响应开启安全插件后会先缓冲再扫描
