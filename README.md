...
`image_request_timeout_sec` 也是 `config.json` 中的隐藏配置项，默认值为 `300`。它仅作用于 `/v1/images/generations`、`/v1/images/edits` 以及 CLI/MCP 复用的本地图片调用链路，用来把图片请求超时放宽到比聊天接口更长的时间；该项不会在设置页单独展示。`akm-image` MCP 工具在未显式传 `timeout` 参数时，也会默认读取这项配置。

如果需要把图片能力接成 MCP 工具，可以直接调用：

- `akm image generate <prompt>`
- `akm image edit <image_path> --prompt <prompt>`

这两个命令都会请求本地代理，成功时只输出原始 JSON，便于外部程序直接解析 `data[].url` 或 `data[].b64_json`。其中 `image edit` 会自动把本地文件组装为 `multipart/form-data`，外部调用方无需自己处理上传细节。

如果你希望直接给 Claude Desktop 等 MCP 宿主使用，项目内还提供了一个零额外依赖的 stdio MCP server：`scripts/mcp_image_server.py`。
...
```json
{
  "mcpServers": {
    "akm-image": {
      "command": "python",
      "args": ["/Users/nk/Desktop/ccs/scripts/mcp_image_server.py"]
    }
  }
}
```
...
`model_matcher` 的 `aliases` 配置除显式映射（如 `gpt-4=gpt-4.1`）外，还支持 `default=目标模型` 作为兜底规则：当聊天协议请求（`chat/completions`、`responses`、`messages`）里的 `model` 未命中任何显式别名时，会统一回退到该默认模型。图片、embeddings 等非聊天请求不会触发这个默认映射。显式映射优先级高于 `default`。
