...
以上和供应商相关的兼容开关已收口到 `provider profile`：当前内置 `openai` 会开启 `max_completion_tokens` / `reasoning_effort` 补齐，其他供应商默认走保守策略，避免在协议层散落硬编码分支。
...
- `thinking` / `reasoning_effort` 的默认补齐也已收口到 `provider profile`：当前仅 `deepseek` 会在未显式传入时补 `thinking={type:enabled}` 与 `reasoning_effort="high"`
