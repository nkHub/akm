...
协议转换由三格式源适配器实现，每个类以源格式命名，按 `api_path` 自动选择：

| 适配器 | 文件 | 路径 | 请求转换 | 响应 SSE 转换 |
|--------|------|------|----------|---------------|
| `ResponsesAdapter` | `adapters/responses_adapter.py` | `/v1/responses` | Responses → Chat | Chat SSE → Responses SSE |
| `MessagesAdapter` | `adapters/messages_adapter.py` | `/v1/messages` | Messages → Chat | Chat SSE → Messages SSE |
| `ChatAdapter` | `adapters/chat_adapter.py` | `/v1/chat/completions` | 透传 | 透传 |

各适配器通过 `agent.py` 懒加载：`agent.responses_adapter`、`agent.messages_adapter`、`agent.chat_adapter`。

#### Responses → Chat 发送方向

`ResponsesAdapter.convert_request()` 将 Codex 的 Responses 请求转为 DeepSeek Chat 格式：
...
- 消息重排序：`system` 移到 `assistant(tool_calls)` 之前，避免 DeepSeek 拒收
- `_clean_schema()` 递归移除 `additionalProperties` / `strict` 字段（DeepSeek 不支持）
- `tool` 结尾时追加空 `user` 消息，触发 DeepSeek 继续生成
- `reasoning_effort` 兜底注入 `"high"`（v4-pro 原生值，绕过服务端兼容映射）

#### Chat SSE → Responses SSE 接收方向

`ResponsesAdapter.convert_sse_stream()` 将 DeepSeek Chat SSE 转为 Responses SSE 事件流：
...
推理内容通过独立的 `reasoning` 类型输出项流式推送，Codex 在「思考」面板中折叠展示，与正文 `output_text.delta` 分离。纯推理（无正文）时推理内容作为 message 兜底输出。流结束时的 `response.completed` 输出数组包含推理项 + 消息项 + 工具调用。
