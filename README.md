...
| `request_redactor` | filter | | 请求侧单向脱敏：把 API Key / Token / PII 等敏感明文替换为稳定占位符后再转发上游（内置默认启用） |
...
`request_redactor` 是一个内置且默认启用的过滤插件，设计目标只有一个：让上游模型看不到敏感明文。首次启动时，插件系统会把它的启用状态写入 `~/.akm/config.json` 的 `plugin_states`。启用后，它会在 `on_request` 阶段递归扫描请求体，对 `messages[].content`、`input`、`instructions`、`tools[].function.arguments` 等文本路径中的常见秘钥、Token、邮箱、手机号、IP、UUID、连接串等内容做稳定占位符替换，格式默认为 `__AKM_<CATEGORY>_<hash12>__`；其中 `hash12` 基于本机 `~/.akm/secret.key` 参与 HMAC 计算，因此同一台机器上的同一明文会稳定映射，但外部无法直接预测。与此同时，`api_key`、`authorization`、`cookie` 等敏感字段会被整字段压缩成同样风格的稳定占位符。该插件不做响应还原，因此更适合“上游不可见”而又希望模型仍能感知“这是同一个值”的场景。
