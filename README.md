...
`model_matcher` 的 `aliases` 配置除显式映射（如 `gpt-4=gpt-4.1`）外，还支持 `default=目标模型` 作为兜底规则：当聊天协议请求（`chat/completions`、`responses`、`messages`）里的 `model` 未命中任何显式别名 **且该模型在 key 池中无可用 key 时**，才会回退到该默认模型。图片、embeddings 等非聊天请求不会触发这个默认映射。显式映射优先级高于 `default`：若显式映射匹配，直接使用映射结果，不触发 default。
