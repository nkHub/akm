...
| `log_request_body` | `false` | 是否记录请求体与请求头快照（含完整对话内容，占用空间大；转换后请求体仅在发生协议转换时记录） |
