...
| 审计 | 请求日志（输入/缓存/输出 Token 列、Key/状态筛选、成功/失败切换、筛选持久化、正倒序、每页 10 条、JSON/会话 WebComponent 渲染、超长内容阈值控制；Token 来源徽标：CT/AF/EST） |
...
为提升跨协议统计稳定性，审计日志中的 Token 采用统一 usage 构建器，按以下优先级回填：

1. 上游标准 `usage`（直接解析）
2. `CT` (`count_tokens_fallback`)：Anthropic Messages `messages/count_tokens` 回填
3. `AF` (`usage_fallback_adapter`)：协议适配器收尾 `_last_usage_tokens` 回填
4. `EST` (`usage_estimated_light`)：轻量估算兜底（仅可观测，不用于计费）

> 说明：Messages 缓存相关字段兼容 `cache_read_input_tokens`（命中）与 `cache_creation_input_tokens`（创建）。
