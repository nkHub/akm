...
`image_request_timeout_sec` 也是 `config.json` 中的隐藏配置项，默认值为 `300`。它仅作用于 `/v1/images/generations`、`/v1/images/edits` 以及 CLI/MCP 复用的本地图片调用链路，用来把图片请求超时放宽到比聊天接口更长的时间；该项不会在设置页单独展示。
...
`model_matcher` 的 `aliases` 配置除显式映射（如 `gpt-4=gpt-4.1`）外，还支持 `default=目标模型` 作为兜底规则：当请求里的 `model` 未命中任何显式别名时，会统一回退到该默认模型。显式映射优先级高于 `default`。
