...
如果希望 `markdown_kb` 插件真正启用 `sqlite-vec` 做向量召回，需确保 Python 内置 `sqlite3` 支持 `enable_load_extension()`。详细配置见 [plugins/markdown_kb/README.md](plugins/markdown_kb/README.md)。
...
akm key usage-query <别名>       # 查询 Key 用量/余额（使用已配脚本）
akm key set-usage-script <别名>  # 设置用量查询脚本（交互式 JSON 编辑）
akm key set-usage-interval <别名> <分钟> # 设置自动查询间隔（分钟，0=关闭）
...
akm plugin enable <名称>      # 启用插件（服务在线时热生效）
akm plugin disable <名称>     # 禁用插件（服务在线时热生效）
...
| 🟢/🟡/🔴 状态 | 运行中 / 启动中 / 失败 |
| 应用管理 | 浏览器打开 Web 管理台 |
| 检查更新 | 立即检查 GitHub Release，有更新弹窗展示 Release Note 确认安装，无更新提示已是最新 |
| 关于 AKM | 弹出原生关于框：logo + 软件名称 + 版本号，可一键复制版本号 |
| 重启服务 | 停止并重新启动代理（端口变更后生效） |
| 退出 | 退出应用 |

启动时从 `~/.akm/config.json` 读取配置（端口、是否自动打开管理台等）。

当前版本的菜单栏应用在 macOS 从休眠恢复后，会自动执行一次分级自愈检查：先等待约 8 秒让网络、VPN 和本地路由恢复，再检查本地服务端口与 `/health/ready`。如果发现本地服务线程未就绪或端口不可达，AKM 会自动重启本地代理服务；如果本地服务已经 ready，则会继续挑选一个已启用且带模型列表的 key，复用现有连通性测试链路发起一次真实上游最小探活。**只有“本地服务未 ready”才会触发自动重启**；上游探活失败只会记录 `wake.recovery.upstream_degraded` 告警并保持本地服务运行，因为上游不通属于网络或对端链路问题，重启本地代理无法修复，反而会打断进行中的请求并重跑插件生命周期。
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>请求日志（输入/缓存/输出 Token 列；开启费用估算后在状态前显示每条估算费用，费用列带图标，悬停展示输入/缓存/输出三者的 Token 数、价格明细及缓存命中率；表格列宽按百分比分配，窄窗口只省略号截断、不横向溢出，超长字段悬停显示全文，来源列对未识别客户端直接展示 UA 产品名；Key/状态筛选、成功/失败切换、筛选持久化、正倒序、每页 10 条、JSON/会话 WebComponent 渲染、超长内容阈值控制；日志头可查看 token 回填来源 flags）</td>
    </tr>
    <tr>
...
控制 AKM 访问上游供应商时的并发连接数，通过 `config.json` / CLI 调整（设置页不单独展示）。
...
控制请求转发时的 Key 选择与重试策略，通过 `config.json` / CLI 调整（设置页不单独展示）。
...
如果需要给本地智能体接入 Markdown 知识库文档同步能力，仓库提供 `skills/markdown-kb-auto-sync/SKILL.md`，详见 [plugins/markdown_kb/README.md](plugins/markdown_kb/README.md)。
...
| GET | `/api/keys/{alias}/usage-config` | 获取用量查询配置 |
...
| PUT | `/api/agents/{name}` | 更新自定义供应商代理（不可改名） |
...
| GET | `/api/plugin-market` | 插件市场列表（plugins/plugins.json 索引，带 5 分钟缓存） |
| POST | `/api/plugin-market/{name}/install` | 从市场索引 zip_url 下载插件 zip（sha256 校验）并覆盖安装/更新 |
| GET | `/api/plugin-market/progress` | 查询插件市场安装/更新进度（前端进度条轮询） |

## Agent Loop

`POST /v1/agent` 提供多轮 LLM 工具调用编排能力：请求传入对话历史，以及可选的工具白名单或 `tool_options` 轻量开关，Agent Loop 内部循环调用 LLM → 解析 `tool_calls` → 执行工具 → 回传结果，直到 LLM 返回最终文本回复或达到最大轮次。Chat 可通过 `tool_options.search` 控制联网搜索、通过 `tool_options.image` 控制图片生成和编辑；`akm_read_image` 不受这两个 UI 开关控制，但只有 `agent_read_image_enabled=true` 时才会被服务端注册并自动注入。其余工具同样按当前注册状态及 `agent_*` 配置开关自动注入，新增或关闭普通工具不再需要更新前端 schema。每次 LLM 调用通过 `proxy.forward_request` 透传，自动复用 Key 选择、协议转换、重试等所有现有能力。内置 `akm_ask_user` 澄清工具支持**交互式提问**：AI 认为信息不足时可中断本轮、向用户反问确认（支持自由输入框 / 单选 / 多选三种模式，由 `options` / `multiple` 参数控制），用户回答后携带上下文续跑（非流式返回 `ask_user` 字段，流式下发 `ask_user` 事件）。流式模式下每个自然停顿点（当前轮输出完整收尾处）会下发 `turn_pause` 事件并携带工作消息快照，客户端可在回复中途插入引导（打断换方向或补充内容）后带着快照续跑，不会产生半截残话。
...
`protocol_converter` 负责 Responses / Messages / Chat 三种格式的双向转换，按 `api_path` 自动选择适配方向。详细转换规则见 [plugins/protocol_converter/README.md](plugins/protocol_converter/README.md)。
