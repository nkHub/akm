...
`stats_include_estimated_usage` 仅作为 `config.json` 隐藏配置项存在，默认 `false`，不会在设置页单独展示；如需让首页统计把 `usage_estimated_light` 这类估算 token 计入总量，可手动改为 `true`。
...
- 若 `data_filter_guard` 开启了“必须整段缓冲”的流式安全模式（常见于 `mask`），插件还会额外受 `stream_guard_buffer_max_bytes` 限制；超过上限时会直接返回安全占位流，避免安全缓冲把服务内存拖死
...
| `data_filter_guard` | filter | | 内置数据安全插件：请求脱敏、关键词/正则过滤、响应风险拦截 |

插件位于 `akm/plugins/`（内置）、项目根目录 `plugins/`（项目本地）或 `~/.akm/plugins/`（第三方），通过管理台「插件」页面启用/禁用/上传。`model_matcher` 标记为必需（`required: true`），不可禁用。

`data_filter_guard` 现已迁入内置插件目录 `akm/plugins/data_filter_guard/`，并保持“内置但默认关闭”；需要时可在插件页手动开启。它作为“数据安全插件”，目前提供以下能力：

- 按字段名递归脱敏，如 `api_key`、`authorization`、`password`
- 内置保守默认配置：默认覆盖常见凭据字段名，以及常见命令执行/脚本投递类响应风险模式
- 按关键词/正则规则替换文本内容，如手机号、邮箱、身份证、银行卡、常见 API Key / JWT；默认关键词仅保留少量高确定性前缀替换
- 可选轻量代码敏感识别：覆盖 OpenAI / Anthropic / GitHub / GitLab / AWS / Google / SendGrid / Slack / JWT / 私钥 / 连接串等开发场景高确定性泄漏模式，支持 `warn` / `mask` / `block`
- 代码敏感规则已拆为分组：`llm_keys`、`vcs_tokens`、`cloud_keys`、`chatops_tokens`、`auth_tokens`、`private_keys`、`db_urls`、`credential_assignments`，可按场景启用
- 插件页已内置代码敏感模板：`开发基线`、`严格阻断`、`环境变量泄露`，可一键填充分组与动作配置
- 如需只关注前端泄漏面，可使用 `前端基线`（`llm_keys,vcs_tokens,cloud_keys,chatops_tokens,auth_tokens`）；后端场景可用 `后端基线`（额外包含 `private_keys`、`db_urls`、`credential_assignments`）
- 插件配置区现已按 `请求脱敏 / 代码敏感识别 / 响应防护` 三段组织，常用开关与阈值会直接显示在结构化面板中
- 对非流式响应执行基础高风险内容扫描，支持整条拦截或局部替换明显恶意脚本/命令片段
- 对响应规则支持动作分级：`warn` / `mask` / `block`
- 流式响应拦截默认关闭；开启后，`warn` / `block` 走增量扫描并保留最近一段缓存，避免阻塞消息返回；仅 `mask` 模式仍需要整段缓冲
- 插件页内置安全模板，可快速套用“命令执行风险 / 凭据泄露风险 / 提示词注入”规则集
- 审计页会额外展示安全事件标记（如 `warn` / `mask` / `block`）
